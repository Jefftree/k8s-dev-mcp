...
import { buildKepIndex, filterKeps, formatKepLine, readKepREADME, readKepYAML } from "./keps.js";
...
// --- list-keps ---
server.tool("list-keps", "List Kubernetes Enhancement Proposals. Filter by SIG name, status, or search query.", {
...
// --- read-kep ---
server.tool("read-kep", "Read the full KEP design document (README.md) by its number.", { kepNumber: z.number().describe("The KEP number, e.g. 4008") }, async ({ kepNumber }) => {
...
// --- read-kep-metadata ---
server.tool("read-kep-metadata", "Read kep.yaml metadata: status, milestones, feature gates, authors.", { kepNumber: z.number().describe("The KEP number, e.g. 4008") }, async ({ kepNumber }) => {
    const kep = kepIndex.find((k) => k.kepNumber === kepNumber);
    if (!kep) {
        return { content: [{ type: "text", text: `KEP ${kepNumber} not found` }], isError: true };
    }
...
// --- read-doc ---
server.tool("read-doc", "Read a Kubernetes convention/guideline document, optionally a specific section.", {
...
// --- list-doc-sections ---
server.tool("list-doc-sections", "List section headings in a document.", { name: z.string().describe('Doc name, e.g. "api-conventions"') }, async ({ name }) => {
    try {
...
// --- search-k8s-docs ---
server.tool("search-k8s-docs", "Full-text search across all configured Kubernetes repos (enhancements, community, website).", {
